...
from func import self_model_forward
import time
...
def main(configFile):
    ...
    #self_model_forward(config=config, model=dreamer.selfModel.eval(), observationShape=observationShape, data=torch.as_tensor([0, 0, 0, 0, 0, 0, 0]), initializeLatents=True)  # save one random latent state for Dreamer start, (eze)
